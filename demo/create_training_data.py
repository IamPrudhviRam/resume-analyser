...
class AnnotatorGui(Frame):
    ...
    def build_line(self, table_content, line_index, line):
        ...
        self.master.create_window(50, line_index * 30, height=30, width=50, window=line_index_label)
        line_content_text = Text(self.master, width=100, height=1)
        line_content_text.insert(INSERT, line_content)
        self.master.create_window(1000, line_index * 30, height=30, width=1800, window=line_content_text)
        ...
        self.master.create_window(1150, line_index * 30, height=30, width=150, window=line_type_button)
        line_label_button = Button(self.master, text='Label: Unknown', width=20,
                                   command=lambda: line_label_button_click(line_index))
        self.master.create_window(1300, line_index * 30, height=30, width=150, window=line_label_button)
        if line[1] != -1:
...
def gui_annotate(training_data_dir_path, index, file_path, file_content):
    ...
    def callback():
        ...
        print(filename)
        output_file_path = os.path.join(training_data_dir_path, filename + '.txt')
