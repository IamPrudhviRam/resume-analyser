...
class ResumeParser(object):
    ...
    def fit(self, training_data_dir_path, model_dir_path, batch_size=None, epochs=None,
        ...
        line_type_history = self.fit_line_type(training_data_dir_path, model_dir_path=model_dir_path,
                                               batch_size=batch_size, epochs=epochs, test_size=test_size,
                                               random_state=random_state)
        print("line_label_history",line_label_history)
        print("line_type_history",line_type_history)
        ...
        for p in texts:
            if len(p) > 10:
                ...
                if name is not None and self.name is None:
                    self.name = name
                    unknown = False
                if email is not None and self.email is None:
                    self.email = email
                    unknown = False
                if sex is not None:
                    self.sex = sex
                ...
                if experience is not None:
                    ...
                    unknown = False
                if mobile is not None and self.mobile is None:
                    #print("m values",mobile)
                    #print("m values self", self.mobile)
                    self.mobile = mobile
                    unknown = False
